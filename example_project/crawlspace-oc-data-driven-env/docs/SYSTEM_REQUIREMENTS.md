...
- `exits`: Array of exit definitions (see below)
- `objects`: Array of object definitions
- `creatures`: Array of creature IDs that can spawn here

**Exit Structure:**
- `name`: Direction or label (e.g., "north", "door to cellar")
...
- `conditions`: Requirements to use (key, stat check, etc.)
- `senses`: Visibility rules (requires light, darkness, etc.)
...
```
- `see_through`: Whether vision can pass through this exit
- `see_cost`: Intensity reduction for sight (sight only reaches adjacent rooms regardless)
